...
const Page = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, helpers) => {
      try {
        await auth.signIn(values.user, values.pass);
        router.push("/");
      } catch (err) {
            ...
            {method === "email" && (
              <form noValidate onSubmit={formik.handleSubmit}>
                ...
                <Alert color="primary" severity="info" sx={{ mt: 3 }}>
                  <div>Recuerda que puedes recuperar tu acceso desde la opción recuperar.</div>
                </Alert>
              </form>
            )}
          </div>
